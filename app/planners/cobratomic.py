...
class LogicalPlanner:
    ...
    def __init__(self, operation, planning_svc, stopping_conditions=()):
        ...
        self.state_machine = ['cobratomic']
        self.next_bucket = 'cobratomic'   # Repeat this bucket until we run out of links.

        # Initialize per-agent step index
        self.agent_steps = {agent.paw: 0 for agent in self.operation.agents}

        # Set up logging
        self.logger = logging.getLogger('cobratomic_planner')
    ...
    async def execute(self):
        self.logger.debug("Executing cobratomic planner")
        await self.planning_svc.execute_planner(self)

    async def cobratomic(self):
        self.logger.debug("Entering cobratomic bucket")
        links_to_use = []
